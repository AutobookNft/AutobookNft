...
class Items extends Component
{
    ...
    public $progress, $totalFiles, $step_precess, $udm, $allowedTypes;
    ...
    public function singleStore($file)
    {
        ...
        $this->allowedTypes = config('AllowedFileType.collection.allowed');
        $maxSize = config('AllowedFileType.collection.max_size');

        $validator = Validator::make(
            ['file' => $file],
            ['file' => 'file|mimes:' . implode(',', array_keys($this->allowedTypes)) . '|max:' . $maxSize]
        );
        ...
        if ($validator->fails()) {
            ...
            $filetypo = $this->allowedTypes[$file->getClientOriginalExtension()];
