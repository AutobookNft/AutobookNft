            ...
            <form wire:submit.prevent="upload()" wire:loading.attr="disabled">
                ...
                <div x-data="{ fileName: null, filePreview: null }" class="grid grid-cols-1 mb-4 bg-gray-500">
                    ...
                    <p class ="counded-t-b p-4 text-sm bg-gray-300 text-gray-800">
                        {{ __('In base alle dimensioni e al numero dei file e al tuo hardware, questa operazione potrebbe richiedere alcuni minuti, non interromperla') }}
                    </p>

                     <p class ="counded-t-b p-4 text-lg bg-gray-300 text-red-800">
                        {{ __('dimensione massima di una singola immagine 12mb') }}
                    </p>
                    ...
                    <div x-data="{ isUploading: false, progress: 0 }"
                        ...
                        <div class="px-2  text-gray-600 text-lg font-bold" wire:loading wire:target="files">{{ __('Uploading') }}</div>
                    ...
                    <div>
                        {{-- @if ($progress == 0)
                                @php ($cls="text-green-500 w-0 shadow-none flex flex-col text-center whitespace-nowrap justify-center bg-green-500")
                            @elseif ($progress == 1)
                                @php ($cls="text-green-500 w-1/3 shadow-none flex flex-col text-center whitespace-nowrap justify-center bg-green-500")
                            @elseif ($progress == 2)
                                @php ($cls="text-green-500 w-2/3 shadow-none flex flex-col text-center whitespace-nowrap justify-center bg-green-500")
                            @elseif ($progress == 3)
                                @php ($cls="text-green-500 w-full shadow-none flex flex-col text-center whitespace-nowrap justify-center bg-green-500")
                        @endif --}}
