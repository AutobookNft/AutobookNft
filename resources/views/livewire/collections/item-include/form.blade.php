<div class="flex flex-col gap-2">
    <form wire:click="edit" id='form-id' wire:submit.prevent="edit" x-data="{ isDisabled: true }"> @csrf
        ...
        <div :class= "{'bg-gray-600':!isDiabled}">
            ...
            {{-- <div class="grid grid-cols-1 m-[1px] border border-red-600"> --}}
            ...
            {{-- <div class="grid grid-cols-1 m-[1px]"> --}}
                <div class="col-span-12">
            ...
            {{-- <div class="grid grid-cols-1 m-2"> --}}
                ...
                <input @keydown="isDisabled = false" type="date"
                    class="shadow appearance-none border rounded text-lg w-full py-1 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
            ...
            <div class="grid grid-cols-2 py-1">
                <label class="relative cursor-pointer">
                    <input @change="isDisabled = false" type="checkbox" id="show" name="show" class="sr-only peer p-2 mt-2 ml-2" checked
                        wire:model.defer="state.show">
                    <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white
                        ...
                        <span class="ml-[50px] text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('Publish') }}</span>
                    </div>
                ...
                <label class="relative cursor-pointer">
                    <input @keydown="isDisabled=false" type="text" id="position" wire:model.defer="state.position"
                        :disabled="!Gate::check('update', $team)" class="rounded-lg bg-gray-200 h-6 text-sm w-1/5">
                    <span class="text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('Position') }}</span>
                </label>
